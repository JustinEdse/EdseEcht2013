...
public class Loader
{
	static double centerLat = 0.0; // latitude and longitude close to Boston
									// marathon bomb, only on static instance of
									// locations.
	...
	public static void main(String[] args) throws Exception
	{
		...
		// 1. I know that several rumors were reported over twitter and even
		// picked
		...
		// the terrorists were foreign/not living in the U.S or were middle
		// eastern,
		// etc. Is there a way that misinformation vs actual fact could be
		// measured?
		// Is there a way to tell how much influence certain users or how many
		// people
		// a rumor spreads to vs when the news outlets pick up the story.
		// Could this be put into an analysis to see percentages or graphs of
		// facts
		// and non facts that came out of the attacks. Just kicking different
		// ideas
		// around.
		List<Tweet> tweets = null;

		// If doing anlaysis on the boston marathon and aftermath, the file
		...
		/*
		 * try { File file = new File("./src/movedUsers.json"); if
		 * (!file.exists()) { file.createNewFile(); } FileWriter fw = new
		 * FileWriter(file.getAbsoluteFile()); BufferedWriter bw = new
		 * BufferedWriter(fw); bw.write(gson.toJson(movedUsers)); bw.close(); }
	...
	private static CellProcessor[] getProcessors()
	{
		...
		final CellProcessor[] processors = new CellProcessor[] { null, // big
																		// int
																		// tweet
																		// id
	...
	public static List<Tweet> loadTweetsFromFile(String path) throws Exception
	{
		...
		centerLat = 42.3497630; // latitude and longitude close to Boston
								// marathon
		...
		try
		{
			...
			// class and
			// each tweet that comes along is check to see if it is within the
			// acceptable range
	 ...
	 * 
	 * 
	 * try { //18:49 GMT, 2:49PM April 15th. String queryString =
	 * "(marathon and boston) AND prayer"; String otherQuery =
	 * "tweet_text: (+boston + finish line"; String escapeQuery =
	 * "tweet_text: that\\s"; BasicSearch newSearch = new BasicSearch();
	 * 
	 * Hits returnedHits = newSearch.performSearch(queryString);
	 * 
	 * @SuppressWarnings("unchecked") Iterator<Hit> iteratorHits =
	 * returnedHits.iterator();
	 * 
	 ...
	 * 
	 * while(iteratorHits.hasNext()) { Hit hit = iteratorHits.next();
	 * 
	 * Document returnedDoc = hit.getDocument();
	 * System.out.println(returnedDoc.get("time") + " " +
	 * returnedDoc.get("tweet_text") + " (" + hit.getScore() + ")");
	 * 
	 * // From here the score gives the user a good indication // of the quality
	 * of each document that was associated // with each hit.
	 * 
	 * }
	 * 
	 * 
